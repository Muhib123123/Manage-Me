...
const FREE_FEATURES = [
    ...
    { text: "Basic 24H Analytics", included: true },
    ...
    { text: "Full Historical Analytics", included: false },
    { text: "Real-time Live Counters", included: false },
];
...
const CREATOR_FEATURES = [
    ...
    { text: "Basic 24H Analytics", included: true },
    { text: "Full Historical Analytics", included: true },
    { text: "Real-time Live Counters", included: true },
...
const COMPARISON = [
    ...
    { label: "Analytics", free: "24h Basic", creator: "Full History + Live" },
];
                    ...
                    <div className="mt-7 flex flex-col items-center">
                        ...
                        {!promoOpen ? (
                            <button 
                                ...
                                className="w-full text-center text-[11px] font-medium text-[var(--accent)] hover:text-amber-500 hover:underline mt-4 bg-transparent border-none cursor-pointer transition-colors"
                            >
                                Have a promo code?
                            </button>
                        ) : (
                            <form onSubmit={handlePromoSubmit} className="mt-4 w-full flex flex-col gap-2 animate-in fade-in slide-in-from-top-2 duration-300">
                                <div className="flex gap-2">
                                    <input 
                                        ...
                                        className="flex-1 bg-[var(--surface-2)] border border-[var(--border-solid)] rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[var(--accent)] text-[var(--text)] transition-colors"
                                        disabled={promoLoading || promoSuccess}
                                    />
                                    <button 
                                        type="submit" 
                                        disabled={promoLoading || promoSuccess || !promoCode.trim()}
                                        className="bg-[var(--accent)] text-white px-4 py-2 rounded-lg text-sm font-semibold disabled:opacity-50 transition-opacity"
                                    >
                                        {promoLoading ? "..." : "Apply"}
                                    </button>
                                </div>
                                {promoError && <p className="text-red-500 text-[11px] font-medium px-1">{promoError}</p>}
                                {promoSuccess && <p className="text-emerald-500 text-[11px] font-medium px-1">Code applied! Upgrading your account...</p>}
                            </form>
                        )}

                        <p className="text-center text-[10px] text-[var(--muted)] mt-5 relative w-full pt-4 before:absolute before:inset-x-0 before:top-0 before:h-px before:bg-[var(--border-solid)]">
