...
export function InstagramUploadForm() {
    ...
    const [mediaPreviewFrame, setMediaPreviewFrame] = useState("");
    ...
    const captureVideoFrame = () => {
        ...
        if (!video || !canvas || postType !== "video") return;
        try {
            ...
            if (ctx) {
                ...
                setMediaPreviewFrame(canvas.toDataURL("image/jpeg", 0.75));
            }
        } catch {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!mediaUrl) { setError("Please upload a photo or video."); return; }
        ...
        try {
            ...
            // Note: We'll create the Instagram endpoint later.
            // Using the existing YouTube endpoint structure as a placeholder for now.
        ...
        <div className="p-6 md:p-10 max-w-[1000px] mx-auto">
            ...
            <div className="mb-12 md:mb-16">
                ...
                <p className="text-[var(--muted)] text-sm md:text-base max-w-2xl leading-relaxed">
                    Upload your photos or videos and set exactly when they go live on your feed.
                </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-10 md:gap-14">
                ...
                {/* ── Card: Post Type ───────────────────── */}
                <FormCard title="1. Select Media Type">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {([
                            { value: "image", label: "Photo Post", desc: "Single square or portrait image" },
                            { value: "video", label: "Video / Reel", desc: "Short-form video up to 90s" },
                            ...
                            >
                                <div className={`font-semibold text-base ${postType === value ? "text-[var(--text)]" : "text-[var(--text)]"}`}>
                                    {label}
                                </div>
                ...
                {/* ── Card: Upload Files ─────────────────── */}
                <FormCard title={`2. Upload ${postType === 'image' ? 'Photo' : 'Video'}`}>
                        ...
                        <div className="flex flex-col gap-4">
                            ...
                            {mediaUrl ? (
                                <UploadedMediaCard
                                    ...
                                    previewFrame={mediaPreviewFrame}
                                    videoRef={videoRef}
                                    onVideoLoaded={captureVideoFrame}
                                    onRemove={() => {
                                        setMediaUrl(""); setMediaProgress(0);
                                        setMediaFileSize(0); setMediaFileName("");
                                        setMediaPreviewFrame("");
                                        setIsUploading(false);
                                    }}
                                />
                            ) : (
                                <UploadZone
                                    endpoint={postType === 'image' ? "thumbnailUploader" : "videoUploader"}
                                    label={`Upload ${postType === 'image' ? 'image' : 'video'}`}
                                    hint={postType === 'image' ? "JPG, PNG — Max 8MB" : "MP4, MOV — Max 2GB"}
                                    progress={mediaProgress}
                    ...
                    <div className="flex flex-col gap-8">
                        <div className="flex flex-col gap-2.5">
                            ...
                            <textarea
                                ...
                                maxLength={2200}
                                rows={6}
                                placeholder="Write a caption for your post. Include hashtags and mentions."
                ...
                {submitStatus === "done" && (
                    <div className="px-5 py-4 rounded-xl bg-[var(--surface-2)] text-[var(--text)] text-sm font-medium border border-[var(--border-solid)] flex items-center gap-3">
                        ...
                        Post successfully scheduled! Redirecting...
                    </div>
                )}
